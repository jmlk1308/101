...
function logout() {
    if (confirm("Are you sure you want to logout?")) {
        ...
        window.location.href = 'admin-login.html'; // ✅ Updated from student-login.html
    }
}
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (currentUser.profilePicture) {
                ...
                img.src = `http://localhost:8080/uploads/${currentUser.profilePicture}`;
...
async function loadNotifications() {
    try {
        const res = await fetch(`${API_URL}/notifications?userId=${getCurrentUserId()}`);
        if (res.ok) {
...
// Get current user ID (in real app, get from JWT)
function getCurrentUserId() {
    // For demo, return 1 (admin user)
    // In production, get from JWT token or localStorage
    return 1;
}
...
async function markNotificationAsRead(notificationId) {
    try {
        await fetch(`${API_URL}/notifications/mark-read/${notificationId}?userId=${getCurrentUserId()}`, {
            method: 'POST'
        });
        loadNotifications();
    } catch (err) {
...
async function markAllAsRead() {
    try {
        await fetch(`${API_URL}/notifications/mark-all-read?userId=${getCurrentUserId()}`, {
            method: 'POST'
        });
        loadNotifications();
    } catch (err) {
...
// ==========================================
// 9. UPDATE EXISTING NAVIGATION
// ==========================================

// Update switchView function to load profile
function switchView(viewId) {
    // Hide all views and remove active class
    document.querySelectorAll('.view').forEach(el => el.classList.remove('active'));
    document.querySelectorAll('.nav-item').forEach(el => el.classList.remove('active'));
    ...
    if (viewId === 'profile-management') loadProfile(); // ✅ NEW
}

// ==========================================
// 10. INITIALIZE EVERYTHING ON LOAD
// ==========================================
